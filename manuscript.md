...
hosts, and ultimately to human (zoonotic spillovers; Plowright \emph{et al.},
...
zoonotic pathogens from wildlife sources (Jones \emph{et al.}, 2008) gives some
...
are likely to happen(Johnson \emph{et al.}, 2020, Albery \emph{et al.}, 2020).
...
and finally, transmission among humans (Lloyd-Smith \emph{et al.}, 2009). In the
...
origin of novel zoonotic viruses remains a major difficulty (Becker \emph{et
...
treatments (Han \emph{et al.}, 2016).  

As suggested by Han \emph{et el.}, a more efficient approach would be
anticipatory. Yet an anticipatory approach can be limited by lack of suitable
data, and as Becker \emph{et al.} (2020) highlighted, by disagreement between
...
decompose a data matrix in a product of three matrices (Ginebreda \emph{et al.},
2019):
...
\textit{Julia} 1.5.1 (Bezanson \textit{et al.} 2017). Truncation at rank $r$ was
...
\textit{not} being imputed are kept at the actual value. We capped the maximal
...
and the aggregation process has been described in Becker \textit{et al.} (2020).
...
\textit{et al.} (2020) dataset used for the model. Therefore, we were able to
...
(identified post Becker \textit{et al.} 2020, according to
\textbf{https://www.viralemergence.org/betacov}). These results are presented in
Table 1 - the novel hosts are presented in \textbf{bold}. Using a rank 2
approximation of the dataset, we have 5 novel hosts, and 4 identified as
"suspected" hosts by the Becker \textit{et al.} (2020) ensemble model, currently
lacking empirical evidence. This suggests that rank 2 contains the most
information about the processes generating the data, and can therefore be used
to infer other associations.
...
\textit{italic}), and 8 out of the 10 interactions are involving the lyssavirus
genus. This genus includes the rabies virus (RABV), and other neurotropic
rabies-related viruses (Warrell \emph{et al.}, 2004). 

Once those results were obtain, further investigations in the form of literature
surveys allowed to identify that the interaction between \textit{Pipistrellus
abramus} and lyssaviruses has already been noted by Hu \emph{et al.} in 2018;
Shipley \textit{et al.} (2019) reported lyssavirus prevalence in the genus
\textit{Pipstrellus}, \textit{Myotis}, and \textit{Rhinolophus}. Other confirmed
hosts of lyssaviruses are \textit{Sus scrofa} (Satou \textit{et al.}, 2004), and
\textit{Rattus norvegicus} (\textit{e.g.} Wang \textit{et al.}, 2014).
...
clinical symptoms has started (Banyard \emph{et al.}, 2017). Although it is
...
poorly understood (Banyard \emph{et al.}, 2017), and these predictions of
...
literature (\textit{Sus scrofa} and orbivirus by Belaganhalli \textit{et al.},
2015; \textit{Capra hircus} and the equine encephalomyelitis caused by an
alphavirus as early as Purcell \textit{et al.}, 1976). This suggests that
...
recommendations on this dataset were validated through \textit{de novo} sampling
...
regard is to rely on Stock \textit{et al.} (2017) work on linear filtering --
